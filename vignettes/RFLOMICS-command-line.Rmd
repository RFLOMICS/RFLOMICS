...
getModelFormula(MAE)
getSelectedContrasts(MAE, all = TRUE)
```
...
```{r contr}
possibleContrasts <- generateExpressionContrast(MAE, contrastType = "averaged")
possibleContrasts$contrastName

MAE <- setSelectedContrasts(MAE, contrastNames = possibleContrasts$contrastName)
...
MAE <- runDataProcessing(
  ...
  lowCountFilter = list(method = "CPM", strategy = "NbReplicates", cpmCutoff = 1),
...
```{r enrich, fig.width=12, fig.height=5, eval=FALSE}
