...
The shiny interface provide a friendly working environment for users who are 
not comfortable enough with R as a language, but still have knowledge about 
omics statistical analyses and data to analyze. 
...
Notice the ".raw" suffix that was added to each of the names. 
This serve as a security measure for the interface pipeline. 
...
Most methods from the RFLOMICS package are designed to work direclty applied to 
...
As always, the process for counts data is a bit different: there is no 
...
form
```
For the rest of the example, we select the second formula:
