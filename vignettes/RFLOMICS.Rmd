...
abstract: > 
  ...
  The first part is showing how to use the user interface. 
  ...
  The second part is showing how to use the package using command lines.
---
...
RFLOMICS is a R package providing a shiny interface. 
It allows the user to analyze transcriptomics, proteomics or metabolomics data, 
from the definition of the statistical model to the multi-omics integration,
and explore the results in a single application. 

In that regard, it is complementary to packages like [ExploreModelMatrix](https://www.bioconductor.org/packages/ExploreModelMatrix), [pcaExplorer](https://www.bioconductor.org/packages/pcaExplorer)
[MatrixQCvis](https://bioconductor.org/packages/MatrixQCvis/) or
[ideal](https://bioconductor.org/packages/ideal/), 
which each allows to perform one particular step of the analysis, or to 
[MSstatsShiny](https://bioconductor.org/packages/MSstatsShiny/)., 
which is designed for one particular omics type. 

The shiny interface provides a friendly working environment for users who are 
not comfortable enough with R as a language, but still have knowledge about 
omics statistical analyses and the data to analyze. 

To use the package, the following input are needed:

* for the interface: tabulated separated files (.tsv)
* for the command-line use: 
    + a data.frame for each omics data
    + a data.frame for the experimental design

At any step of the analysis, a report and all results can be exported and saved. 
The package relies on an extension of the 
[MultiAssayExperiment](https://bioconductor.org/packages/MultiAssayExperiment/) 
class, the results can be loaded into R and analyzed with more classic tools if
needed. 

# Install and run RFLOMICS

You can install the latest RFLOMICS development using the remotes package:
...
This part of the vignette will follow the different steps of a multi-omics analysis trough the 
...
modified using the ggplto2 functions.

Each of the analysis step has their own plot and getters methods. 
Please refer to the "Using RFLOMICS with command line" part of the vignette
to have examples of visualization functions for each steps.

# Using RFLOMICS without the interface

This part of the vignette will focus on the use of RFLOMICS as a command-line package. 
...
All you need is contained in the ecoseed object:
...
Note the ".raw" suffix that was added to each of the names. 
This serves as a security measure for the interface pipeline. 
For this example, we won't use it. 

```{r}
names(testObject) <- gsub("[.]raw", "", names(testObject))
```

Most methods from the RFLOMICS package are designed to work directly applied to 
a RflomicsSE object or to a RflomicsMAE object and indicating the dataset name
with the SE.name argument.
Analysis methods return the same class of object (RflomicsSE or RflomicsMAE) 
as the entry object type with the correct slot or list entry completed with 
the results.

## Pre-processing of data

The next is step is to pre-process data. 
Proteomics and metabolomics data can already be processed, 
but it is important to use counts data for transcriptomics data. 
There are two settings to set: the normalization and the transformation. 
The function `runNormalization` allows you to set the normalization method, 
and the `runTransformData` the transformation one.
...
argument `modifyAssay` to TRUE.
This way, you can always change your mind and test several normalization or 
...
The process for counts data is a bit different: there is no 
transformation method to set, the log2 transformation will be used (when needed)
if the type of omics if set to RNAseq when creating the object. 
...
The next step is to define the model used by all following steps.
The function `generateModelFormulae` finds all the possible models given the 
experimental design:
...
form
```
For the remaining of the example, we select the second formula:

```{r}
testObject <- setModelFormula(testObject, form[2])
getModelFormula(testObject)
```
Given the chosen formula, all contrasts can be computed:
...
Once the contrasts have been set, the differential analysis can be run.
For this example, only the RNAseq data will be used. 
The method (either from edgeR package or limma package) can be manually set or 
left to the function to decide what is the best method to use given the omics type.
...
To explore the results from a differential analysis run, you have to indicate
which contrast you want to consider. You can refer to your selected contrasts 
using the `getSelectedContrasts` function.

```{r}
getSelectedContrasts(testObject)$contrastName
```
...
In this example, we will use only the proteomics and metabolomics dataset with
the mixOmics package (block.plsda function). 
