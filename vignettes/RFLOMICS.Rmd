...
Choice of model formula: RFLOMICS uses the biological and batch factors 
specified in the Load Data module to automatically generate relevant statistical models, 
with or without interactions. Only interactions between experimental factors are considered.
This step is common to all datasets and is essential for both differential analysis and multi-omics integration.

Users have to choose their model formula, which will be applied to all datasets.

Models are in the general form of : 
\[ [1] \sim BioFactor1 + BioFactor2 + batchFactor\]
\[ [2] \sim 1 + BioFactor1 + BioFactor2 + BioFactor1:BioFactor2 + batchFactor\]

Interactions represent effects that cannot be explained by the main effects alone. 
The first model includes only main effects, assuming that factors act independently, 
while the second model also incorporates interactions between experimental factors. 
Batch factors are never included in interaction terms, as they represent technical 
rather than biological variation. Their role is solely to correct for technical 
effects without affecting interactions between biological factors.

Based on the selected model, all 'a priori' hypotheses (contrasts) are then calculated 
...
Three types of contrasts can be computed:

Simple contrasts: Comparison between two levels of a factor while keeping the other factors fixed.
Example: Contrast $$|(temperatureHigh – temperatureLow)| in |ImbibitionDS|$$
Corresponding hypothesis: Is there an effect of temperature (low vs. high) on seed 
production in dry seeds, within a model including the interaction between temperature and imbibition.

Mean contrasts: Comparison between two levels of a factor while averaging other factor levels.
Example: Contrast $$|(temperatureHigh – temperatureLow)| in |mean|$$
Corresponding hypothesis: difference in gene expression between low and high temperature 
regardless of the level of imbibition.

For models without interaction terms, simple contrasts are equivalent to mean contrasts. 
this reflects the effect of a single factor on the response variable, averaged over all 
levels of the other factors, and no interaction contrasts are defined.

Users can select contrasts that precisely reflect their biological questions and 
experimental design. This statistical framework is applied to all loaded datasets.
...
This step evaluates data quality and explores the structure of each dataset. 
It allows users to quickly assess data quality 
through diagnostic plots displayed before and after preprocessing, and to evaluate
the experimental design by exploring and quantifying the main sources of biological
and technical variation (PCA)
...
#### Transcriptomics data (count data)
...
#### Proteomics and metabolomics data (intensity-based data)

- Filtering of variables with a high proportion of missing values % 
- Log2 or log10 transformation
- Median normalization, commonly recommended in the literature [ref] 
- totalsum (no recommended when a small number of feature account for a large 
proportion of the total signal) 
- Imputation of missing values using a simply approaches:
replacing missing values with the minimum observed value in the table.
...
For RNAseq data,  a generalized linear model (GLM) with a negative binomial distribution
is applied: the **glmFit()** function from the **edgeR** package is used.  
For proteomics and metabolomics data, differential analysis relies on linear models
moderated by an empirical Bayesian approach, the **lmFit()** function from **limma** packages is used.
...
This step aims to identify groups of variables with similar expression, abundance, or 
accumulation patterns across different experimental conditions.
...
* Co-expression analysis is done using to the **coseq** package which relies on mixture models
with a set of  adapted parameters 
...
* Results description (see [CoSeq](https://www.bioconductor.org/packages/release/bioc/vignettes/coseq/inst/doc/coseq.html))
  ...
  -  A warning table is also provided, summarizing the number 
of replicates that successfully produced results for each tested number of clusters, 
enabling users to identify potential issues with convergence or insufficient data.
...
Enrichment analysis is performed using Over-Representation Analysis (ORA) which 
employs a hypergeometric test and can be done on differentially expressed variables 
for each contrast or on each clusters of co-expressed variables. 

ORA can be performed using the KEGG or Gene Ontology (GO) databases, subject to
gene nomenclature and coverage. Users can also provide a custom database, for 
example a BioMart extract for a non-model species.

 User have to select the lists of DE genes or clusters to annotate. All available lists 
...
### Compare analysis:
 ...
 A summary of single-omics analyses is generated once at least two datasets have 
 been analyzed. 
 ...
 samples and entities are left in each table. 
 
 If differential analyses have been performed, the distribution of
 up- and down-regulated variables can be compared across contrasts (rows) and 
 between datasets (columns). When co-expression analyses are available, cluster 
 profiles can be compared across datasets, although no statistical tests are applied.

If ORA has been performed on the differential analysis results, enriched biological 
terms can be compared between contrasts and, within a contrast, across datasets. 

This comparison is displayed as a presence/absence heatmap, allowing rapid identification 
of convergent and divergent biological signals across hypothesis tests and datasets, 
depending on the selected database or ontology.
...
## Integration
 ...
 tab will appear in the side bar menu. 
 
 Two sub menus are available. For each of 
...
### MOFA
 ...
 This integration analysis is performed using the **MOFA2** \citep{Argelaguet2020MOFA2}
 R-package. It is an unsupervised method that searches for latent factors to decompose the variance 
...
### DIABLO
 ...
 This integration analysis is performed using the **MixOmics**  \citep{Rohart2017MixOmics} R-package. 
