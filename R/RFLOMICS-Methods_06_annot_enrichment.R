...
setMethod(
    ...
    definition = function(object,
        ...
        returnplot <-  switch(
            plotType,
            "cnetplot" = {
                df1 <- dataPlot@result[dataPlot@result[["Description"]] %in% Categories,]
                sizeCat <- df1[, c("Description", "Count")]
                df1 <- do.call("rbind", lapply(seq(1, nrow(df1)), FUN = function(i){
                    data.frame("Category" = df1$Description[i], "Gene" = unlist(str_split(df1$geneID[i], "/")))
                }))

                incMat <- as.matrix(dcast(df1, Category ~ Gene, fun.aggregate = function(x) 1, fill = 0, value.var = "Gene"))
                if(nrow(incMat) == 1) {
                  incMat <- rbind(incMat, c("Phantom", rep(0, ncol(incMat) - 1)))
                }
                rownames(incMat) <- incMat[,1]
                incMat <- incMat[,-1]

                net <- ggnetwork(incMat)
                if (!is.null(log2FC_vect)) {
                    log2FC_vect <- data.frame("Gene" = names(log2FC_vect), "log2FC" = log2FC_vect)
                    net <- merge(net, log2FC_vect, by.x = "vertex.names", by.y = "Gene", all.x = TRUE)
                    net$log2FC <- as.numeric(net$log2FC)
                }
                net$Count <- sizeCat$Count[match(net$vertex.names, sizeCat$Description)]

                net2 <- net
                # net2$color <- ifelse(net2$vertex.names %in% Categories, "#827081", NA)
                net2 <- net2[which(net2$vertex.names %in% Categories),]
                net2$label <- NA
                net$label <- NA
                net$label[which(!net$vertex.names %in% Categories)]  <- net$vertex.names[which(!net$vertex.names %in% Categories)]
                net2$label[which(net2$vertex.names %in% Categories)] <- net2$vertex.names[which(net2$vertex.names %in% Categories)]

                net <- net[which(!net$vertex.names %in% Categories),]
                net <- net[which(!net$vertex.names %in% c("Phantom")),]
                ...
                if (!is.null(log2FC_vect)) {
                    gg <- gg + geom_nodes(aes(color = log2FC), size = 2) +
                ...
                } else {gg <- gg + geom_nodes(color = "#636363", size = 2)}

                if ("gene" %in% nodeLabel) {gg <- gg + geom_text(color = "grey11", position = position_nudge(y = 0.02))}

                gg <- gg  +
                    geom_nodes(data  = net2, color = "#E3D1D1", aes(size = Count))

                if ("category" %in% nodeLabel) {
                    gg + geom_text(data = net2, color = "black", position = position_nudge(y = 0.075))
                } else {
                    gg
                }

                # Also broken, need ggtangle ?
                # cnetplot(
                #  dataPlot,
            ...
            "heatplot" = {
                ...
                if(!is.null(log2FC_vect)) {
                    log2FC_vect <- data.frame("Gene" = names(log2FC_vect), "log2FC" = log2FC_vect)
                    df1 <- merge(df1, log2FC_vect, by = "Gene", all.x = TRUE)
                }

                gg <- ggplot(data = df1, aes(x = Gene, y = Category))

                if (!is.null(log2FC_vect)) {
                    gg <- gg + geom_tile(aes(fill = log2FC), color = "white") +
                ...
                # Broken in bioc 3.22? Don't seem to work in the app anymore
            ...
            {
                df1 <- dataPlot@result[dataPlot@result[["Description"]] %in% Categories,]
                df1$GeneRatio <- unlist(lapply(df1$GeneRatio,
                                               FUN = function(x) eval(parse(text = x))))
                df1$Description <- factor(df1$Description,
                                          levels = df1$Description[order(df1$GeneRatio, decreasing = FALSE)])

                ggplot(df1, aes(x = GeneRatio, y = Description, color = p.adjust)) +
                    geom_point(shape = 19, aes(size = Count)) +
                    theme_bw() + ylab("") +
                    scale_color_continuous(low = "red",
                                           high = "blue")

                # ggplotly(layerData = 2)
                # tryCatch(
                #     dotplot(dataPlot,
                #             showCategory = Categories,
                #             ...),
