...
test_that("it's running from diffExpAnal - GO - RNASeq", {
  ...
  expect({
    obj <- RFLOMICS:::getEnrichRes(MAE[["RNAtest"]], 
...
# 
#   expect({
#     obj <- RFLOMICS:::getEnrichRes(MAE[["RNAtest"]], contrast = "(temperatureElevated - temperatureLow) in imbibitionDS", 
#                                    database = "GO", domain = "BP")
#     nrow(obj@result) > 0
#   }, failure_message = "(GO RNAseq from DiffExp) - There is no result in the enrichment metadata part.")
# 
