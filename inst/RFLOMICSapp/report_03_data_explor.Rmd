...
filtering <- paste0(
  ...
  length(metadata(dataset.SE)$DataProcessing$rowSumsZero)," transcript).",
...
```{r Library_size_barplot, fig.show = "hold", out.width = "50%", eval=(getOmicsTypes(dataset.SE) == "RNAseq"), fig.cap=paste0("<p style=\"color:grey\">","*fig: Barplot of the library size for each sample before and after the normalization step*", "</p>")}
...
plotOmicsPCA(dataset.SE, raw="raw", axes=c(1, 2), groupColor="groups")
...
plotOmicsPCA(dataset.SE, raw="raw", axes=c(1, 3), groupColor="groups")
